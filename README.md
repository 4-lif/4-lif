...
def masuk():
    ...
    rom = raw_input('\n%s [?] Menu : %s'%(P,K))
    if rom in(""):
    	print("%s [!] Isi yang benar asu "%(M));exit()
    elif rom in ('1','01'):
        jalan("\n%s [%s!%s] wajib akun tumbal asu "%(P,M,P))
    	Mal  = raw_input('%s [?] Token : %s'%(P,K))
        if mal in(""):
        ...
        if nanya in(""):
        	...
        	print ("\n%s [√] selamat anda tolol :* "%(H));jeda(2);masuk()
        elif nanya in("n","N"):
        	print ("\n%s [!] anda sungguh pintar "%(M));jeda(2);os.system("xdg-open https://youtu.be/IG5QfdxRkeY");masuk()
    elif rom in ('0', '00'):
    	exit('\n')
    else:
    	print("%s [!] Isi yang benar asu "%(M));exit()
...
def publik(mal,headers=header):
    ...
    try:
        ...
        exit('\n %s[!] gagal dump id'%(P))
# DUMP FOLLOWERS
def followers(mal,headers=header):
    try:
        os.mkdir('dump')
    except:pass
    try:
        ...
        for a in z['data']:
            ...
            sys.stdout.flush();jeda(0.0050)
        bff.close()
        print ('\n\n %s[%s√%s] berhasil dump id dari %s%s'%(P,H,P,H,nm['name']))
        print (' %s[%s√%s] File dump tersimpan :%s %s '%(P,H,P,H,file))
        raw_input('\n%s [ %senter %s] '%(P,K,P))
        menu()
    except Exception as e:
        exit('\n %s[!] yang bener asu'%(P))
# DUMP POSTINGAN 
def postingan(mal,headers=header):
    try:
        os.mkdir('dump')
    except:pass
    try:
